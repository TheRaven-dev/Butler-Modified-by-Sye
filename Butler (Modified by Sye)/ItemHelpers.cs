...
namespace Butler__Modified_by_Sye_
{
    internal static class ItemHelpers
    {

        private static ItemInfo EquippedItem1 = null;
        private static ItemInfo EquippedItem2 = null;
        private static readonly Dictionary<string, float> itemValues = new Dictionary<string, float>();
        private static readonly Dictionary<int, int> itemValueOverrides = new Dictionary<int, int>();


        public static void checkThisItem(this ItemInfo item)
        {
        ...
        public static float getItemValue(ItemInfo item)
        {
            float itemValue = 0;
            if (item != null)
            {
                ...
                if (itemValue == -1701)
                {
                    ...
                    if (!itemValues.TryGetValue(itemLink, out itemValue))
                    {
                        ...
                        Logging.WriteDebug(Main.ButlerPrefix + "acknowledging item \"" + item.itemName + "\" id " + item.itemEntry + " with a value of " + itemValue);
                    }
                }
            }
            return itemValue;
        ...
        public static void getitemValueOverrides()
        {
        ...
        private static float getItemStatValue(string itemStats)
        {
            ...
            if (itemStats.Length > 1)
            {
                ...
                foreach (string itemStat in statsArray)
                {
                    ...
                    int index = -1;
                    for (var i = 0; i < Main.itemStatConstants.Length; i++) { if (Main.itemStatConstants[i] == statKey) { index = i; }; }
        ...
        {
            string itemStats = Lua.LuaDoString<String>($@"local stats4butler = {{}} or nil
														local istats = GetItemStats({item.itemLink}) 
														for stat, value in pairs(istats) do 
															tinsert(stats4butler, _G[stat] .. ':' .. value .. ';' );
														end
														return unpack(stats4butler)");
            return itemStats.Substring(0, itemStats.Length - 1);
        }

        public static string[] getItemStatConstants()
        {
            string statConstants = Lua.LuaDoString<String>(@"local stats4butler = {};      
															tinsert(stats4butler, ITEM_MOD_AGILITY_SHORT .. ';'.. 
