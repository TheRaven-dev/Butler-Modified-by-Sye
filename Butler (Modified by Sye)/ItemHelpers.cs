    ...
    internal static class ItemHelpers
    {
		public static void checkThisItem(this ItemInfo item)
		{
			if (item.itemMinLevel <= ObjectManager.Me.Level)
			{
				...
				if (itemValue != 0)
				{
					...
					float wert1 = 0;
					float wert2 = 0;
					if (Main.EquippedItem1 != null) { wert1 = getItemValue(item); } else { wert1 = -1; }
					if (Main.EquippedItem2 != null) { wert2 = getItemValue(item); } else { wert2 = -1; }
					if (wert2 > wert1)
					{
						wert1 = wert2;
						Main.EquippedItem1 = Main.EquippedItem2;
					}
					if (itemValue > wert1)
					{
		...
		public static float getItemValue(ItemInfo item)
		{
			...
			if (item != null)
			{
				...
				if (itemValue == -1701)
				{
					...
					if (!Main.itemValues.TryGetValue(itemLink, out itemValue))
					{
						string itemStats = getItemStats(item);
						itemValue = getItemStatValue(itemStats);
						Main.itemValues.Add(itemLink, itemValue);
		...
		private static int getItemOverride(ItemInfo item)
		{
			...
			if (item != null)
			{
				...
				if (!Main.itemValueOverrides.TryGetValue(itemEntry, out itemValue))
				{
					itemValue = -1701;
				}
			}
		...
		public static void getitemValueOverrides()
		{
			...
			foreach (string Key in KeyPair)
			{
				...
				if (converted == 2) { Main.itemValueOverrides.Add(itemEntry, itemValue); }
			}
		}
		...
		{
			bool loadFirst = true;
			Main.EquippedItem1 = null;
			Main.EquippedItem2 = null;
			foreach (ItemInfo item in getEquippedItems())
			{
				if (equipLoc == translateLoc(item.itemEquipLoc))
				{
					if (loadFirst)
					{
						Main.EquippedItem1 = new ItemInfo(item.itemLink);
						loadFirst = false;
					}
					else
					{
						Main.EquippedItem2 = new ItemInfo(item.itemLink);
					}
				}
			}
		}
